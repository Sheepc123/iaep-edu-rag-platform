...
export const TeacherChatRoom = () => {
  ...
  const getMessagesForContact = (contactId: string): ChatMessage[] => {
    const messageMap: Record<string, ChatMessage[]> = {
      '1': [ // 张同学
        ...
        {
          id: '3',
          content: '关于递归算法的时间复杂度分析，我还是不太明白。',
          sender: 'student',
          senderName: '张同学',
        ...
        {
          id: '4',
          content: '好的，我来详细解释一下。递归算法的时间复杂度主要看递归的深度和每层的操作次数...',
          sender: 'teacher',
          senderName: '我',
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const reply: ChatMessage = {
          ...
          sender: selectedContact.role === 'teacher' ? 'teacher' : 'student',
  ...
  // 生成回复
  const generateReply = (teacherMessage: string, role: 'teacher' | 'student'): string => {
    ...
    if (role === 'teacher') {
      ...
      const teacherReplies = [
        '好的，我们可以进一步讨论这个问题。',
        '这个想法很有意思，值得深入研究。',
        '我同意你的观点，我们可以合作完成这个项目。'
      ];
      return teacherReplies[Math.floor(Math.random() * teacherReplies.length)];
    } else {
      // 学生回复教师的情况
      if (lowerMessage.includes('作业') || lowerMessage.includes('练习')) {
        return '好的老师，我会按时完成作业的！';
      }
      if (lowerMessage.includes('考试') || lowerMessage.includes('测试')) {
        return '谢谢老师的提醒，我会好好复习的。';
      }
      if (lowerMessage.includes('问题') || lowerMessage.includes('疑问')) {
        return '谢谢老师的耐心解答，我明白了！';
      }
                ...
                <div className="flex items-end space-x-3">
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      placeholder={`给 ${selectedContact?.name} 发送消息... (Shift+Enter 换行)`}
